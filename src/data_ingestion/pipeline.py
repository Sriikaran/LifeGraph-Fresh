...
from data_ingestion.excel_loader import load_products
...
def enrich_products_pipeline() -> Dict[str, Any]:
    ...
    for p in products:
        ...
        mission_hints = []
        for h in (p.missionHints or []):
            h_clean = h.lower().strip()
            if h_clean in APPROVED_MISSIONS and h_clean not in mission_hints:
                mission_hints.append(h_clean)
        ...
        embedding_text = f"title: {title} | category: {category} | subcategory: {subcategory} | semanticTags: {', '.join(semantic_tags)} | missionHints: {', '.join(mission_hints)}"

        # Update attributes
        p_dict["category"] = category
        p_dict["subcategory"] = subcategory
        p_dict["semanticTags"] = semantic_tags
        p_dict["missionHints"] = mission_hints
